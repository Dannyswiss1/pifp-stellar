...
fn test_donation_count_increments_for_different_donors() {
    ...
    ctx.client.deposit(&project.id, &donator1, &token.address, &500i128);
    ctx.client.deposit(&project.id, &donator2, &token.address, &300i128);
...
fn test_donation_count_complex_scenario() {
    ...
    ctx.client.deposit(&project.id, &donator1, &token1.address, &100i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 1);

    ctx.client.deposit(&project.id, &donator1, &token1.address, &100i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 1);

    ctx.client.deposit(&project.id, &donator2, &token1.address, &200i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 2);

    ctx.client.deposit(&project.id, &donator1, &token2.address, &150i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 3);

    ctx.client.deposit(&project.id, &donator3, &token1.address, &300i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 4);

    ctx.client.deposit(&project.id, &donator2, &token2.address, &250i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 5);

    ctx.client.deposit(&project.id, &donator2, &token2.address, &100i128);
    assert_eq!(ctx.client.get_project(&project.id).donation_count, 5);
}
